...
function Sidebar(props) {
  const { cartItems, removeCount, increaseCount, setCartItems } = props;
  const itemPrice = cartItems.reduce((a, c) => a + c.price * c.qty, 0);
  const totalPrice = itemPrice;

  return (
    <div className='mt-14 lg:mr-5 md:ml-2 max-sm:p-8  '>
      {
        cartItems.length === 0 ?
          <h2 className='text-lg text-orange-600'>Your Order List Is <b> empty </b></h2> :
          <h2 className='text-lg text-orange-600'>Your Orders</h2>
      }
      <div className="grid flex-col justify-between items-center border-b-2 mt-4 max-sm:mr-20">
        {
          cartItems.length === 0 ?
            <div className=" max-sm:ml-20 md:ml-10  lg:ml-20">
            ...
            cartItems.map((item) =>
              <table key={item.id} className='gird lg:grid-rows-2 grid-cols-3'>
                <thead className='text-lg font-semibold lg:row-span-1'>{item.name}</thead>
                <tbody className='lg:row-span-2 lg:col-span-1'>
                  <tr>
                    <td className='text-orange-600'>X{item.qty}</td>
                    <td className=''>${item.price}</td>
                    <td>{item.totalPrice}</td>
                    <div className='flex m-3 '>
                      <FaCircleMinus className='mx-1 size-6 text-orange-400 cursor-pointer' onClick={() => removeCount(item)} />
                      <FaCirclePlus className='mx-1 size-6 text-orange-400 cursor-pointer' onClick={() => increaseCount(item)} />
                    </div>
                  </tr>
                </tbody>
              </table>
      ...
      <div className="flex justify-between mt-5 max-sm:mr-20">
        <p>Order Total</p>
        <p>${totalPrice}</p>
      </div>
      {/* Modal */}
